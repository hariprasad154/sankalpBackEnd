...
def record_apply_failure(username: str, job_title: str = "", company: str = "", error: str = "") -> None:
    row = get_user_state(username)
    failed = int(row.get("failed_today", 0) or 0) + 1
    msg = (error or "Apply failed")[:240]
    patch_user(
        username,
        failed_today=failed,
        last_error=msg[:500],
        last_log=msg,
        current_job=(job_title or "")[:500],
        current_company=(company or "")[:255],
    )
...
def build_dashboard_state(username: str) -> dict:
    ...
    return {
        ...
        "failed_today": int(rt.get("failed_today", sheet_stats.get("failed_count", 0)) or 0),
