...
APP_HEADERS = ["username", "company", "role", "status", "error", "timestamp"]
...
def append_application(row: list) -> None:
    """Append Applications row: username, company, role, status, error, timestamp."""
    _append_row(SHEET_APPLICATIONS, row)
...
def save_application(
    ...
    st = status.upper() if status else "FAILED"
    if st in ("APPLIED", "SUCCESS"):
        st = "SUCCESS"
    elif st not in ("SUCCESS", "FAILED"):
        st = "FAILED" if st in ("APPLYUNCERTAIN", "FAIL") else st
    append_application(
        [
            username,
            company[:255],
...
def _normalize_application(row: dict) -> dict:
    ...
    if st.lower() in ("applied", "success"):
        st = "SUCCESS"
    elif st and st.upper() not in ("SUCCESS",):
        st = "FAILED"
    return {
...
def get_applications(username: str, limit: int = 100) -> list[dict]:
    ...
    rows.reverse()
    return [_normalize_application(r) for r in rows[:limit]]
...
def dashboard_stats(username: str) -> dict:
    ...
    failed = 0
    for a in apps:
        d = str(a.get("timestamp") or a.get("applied_at", ""))[:10]
